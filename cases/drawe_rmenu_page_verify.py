...
class DrawerMenuCase(unittest.TestCase):
    ...
    def test_login_succeed_recent_read(self):
        ...
        drawerMenuPage = drawer_menu_page.DrawerMenuPage(self.driver)
        Common.is_login_status_fail(self.driver, ReadUser().get_username, ReadUser().get_password)
        time.sleep(1)
        drawerMenuPage.recent_read
        time.sleep(1)
        drawerMenuPage.screenshot(Common.get_pic_path())
    ...
    def test_night_mode(self):
        ...
        time.sleep(3)
        drawerMenuPage.screenshot(Common.get_pic_path())
        self.assertEquals(drawerMenuPage.night_mode.text, '白天模式')  # 判断切换为夜间模式，显示是否为白天模式
    ...
    def test_daytime_mode(self):
        ...
        time.sleep(3)
        drawerMenuPage.screenshot(Common.get_pic_path())
        self.assertEquals(drawerMenuPage.night_mode.text, '夜间模式')  # 判默认为白天模式，并且是否切换夜间模式
